...
class HPSIImplementations:
    
    @staticmethod
    def hpsi00_original(grids, meanfield, iq, weight, weightuv, pinn):
        """OLD HPSI00"""
    ...
    @staticmethod
    @jax.jit
    def hpsi00_optimized(grids, meanfield, iq, weight, weightuv, pinn):
        '''NEW HPSI00'''
        sigis = jnp.array([0.5, -0.5])
        
        # Pre-compute common terms, maintaining original precision
        spot_iq = meanfield.spot[iq]
        wlspot_iq = meanfield.wlspot[iq]
        aq_iq = meanfield.aq[iq]
        bmass_iq = meanfield.bmass[iq]
        
        # Step 1: Match original implementation's computation order
        pout = jnp.multiply(pinn, meanfield.upot[iq,...])
        
        # Step 2: Spin-current coupling with preserved ordering
        pout = pout.at[0,...].add(
            (spot_iq[0] - 1j * spot_iq[1]) * pinn[1] + 
            spot_iq[2] * pinn[0]
        )
        pout = pout.at[1,...].add(
            (spot_iq[0] + 1j * spot_iq[1]) * pinn[0] - 
            spot_iq[2] * pinn[1]
        )
        ...
        # Final weight adjustments maintaining original precision
        pout_mf = pout
        v_pair_term = weightuv * meanfield.v_pair[iq,...][None, ...] * pinn
        pout = weight * pout - v_pair_term
