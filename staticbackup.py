...
def statichf(coulomb, densities, energies, forces, grids, levels, meanfield, moment, params, static):
    ...
    print("first diagstep done")

    densities = add_density(densities, grids, levels)
    print("add densities done")
...
def harmosc(grids, levels, params, static):
    """
    Initialize wave functions with harmonic oscillator states.
    
    This function initializes all wave functions with harmonic oscillator states.
    First, it calculates the lowest state (a Gaussian), then it generates the 
    higher states by multiplying the Gaussian with appropriate polynomials.
    
    Args:
        grids: Grid parameters
        levels: Level parameters
        params: Parameters containing flags
        static: Static parameters with radinx, radiny, radinz
        
    Returns:
        Updated levels with initialized wave functions
    """
    if params.wflag:
        print("Harmonic oscillator widths (x-y-z):")
        print(f"{static.radinx:12.4f} {static.radiny:12.4f} {static.radinz:12.4f}")
    
    # Initialize arrays
    psi = jnp.zeros_like(levels.psi)
    nstmax = levels.nstmax
    
    # Update occupation numbers
    wocc = jnp.zeros(nstmax)
    wocc = wocc.at[:levels.nneut].set(1.0)
    wocc = wocc.at[levels.nneut:levels.nneut + levels.nprot].set(1.0)
    wguv = jnp.zeros_like(wocc)
    
    # Generate shell structure for states
    nshell = jnp.zeros((3, nstmax), dtype=jnp.int32)
    
    # Shell structure calculation - done in normal Python (not JAX) for clarity
    shell_idx = 0
    for iq in range(2):
        nps = levels.nneut if iq == 0 else levels.nprot
        ...
        for ka in range(nps + 1):
            for k in range(ka + 1):
                for j in range(ka + 1):
                    for i in range(ka + 1):
                        if ka == i + j + k:
                            for _ in range(2):  # Spin states
                                if shell_idx < nps:
                                    nshell = nshell.at[0, shell_idx].set(i)
                                    nshell = nshell.at[1, shell_idx].set(j)
                                    nshell = nshell.at[2, shell_idx].set(k)
                                    shell_idx += 1
    ...
    # Generate the lowest state: Gaussian
    ...
    # Normalize
    norm = jnp.sqrt(jnp.sum(gaussian**2) * grids.wxyz)
    gaussian = gaussian / norm
    
    # Initialize the first state for each isospin
    # Neutrons
    psi = psi.at[0, 0, :, :, :].set(gaussian)
    psi = psi.at[0, 1, :, :, :].set(0.0)
    
    # Protons
    psi = psi.at[levels.nneut, 0, :, :, :].set(gaussian)
    psi = psi.at[levels.nneut, 1, :, :, :].set(0.0)
    
    # Generate higher states for neutrons
    for i in range(1, levels.nneut):
        state_idx = i
        spin_idx = i % 2
        
        # Create polynomial factors
        x_factor = jnp.ones_like(grids.x) if nshell[0, state_idx] == 0 else grids.x**nshell[0, state_idx]
        y_factor = jnp.ones_like(grids.y) if nshell[1, state_idx] == 0 else grids.y**nshell[1, state_idx]
        z_factor = jnp.ones_like(grids.z) if nshell[2, state_idx] == 0 else grids.z**nshell[2, state_idx]
        
        # Create 3D polynomial
        x_factor = x_factor[:, jnp.newaxis, jnp.newaxis]
        y_factor = y_factor[jnp.newaxis, :, jnp.newaxis]
        z_factor = z_factor[jnp.newaxis, jnp.newaxis, :]
        polynomial = x_factor * y_factor * z_factor
        
        # Multiply with gaussian and normalize
        wave_func = gaussian * polynomial
        norm = jnp.sqrt(jnp.sum(wave_func**2) * grids.wxyz)
        wave_func = wave_func / norm
        
        # Set the wave function in the appropriate spin component
        psi = psi.at[state_idx, spin_idx, :, :, :].set(wave_func)
        psi = psi.at[state_idx, 1-spin_idx, :, :, :].set(0.0)
    
    # Generate higher states for protons
    for i in range(1, levels.nprot):
        state_idx = i + levels.nneut
        spin_idx = i % 2
    ...
    if params.wflag:
        print("***** Harmonic oscillator initialization complete *****")
